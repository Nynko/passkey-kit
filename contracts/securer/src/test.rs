...
use smart_wallet_interface::types::{
    Signature, Signer, SignerKey, SignerLimits, SignerStorage,
};
use soroban_sdk::{
...
use crate::{types::{Signatures, SignerPubKey, SimpleMultiSig}, Contract as SecurerContract, SecurerAdminClient};
...
struct Environment<'a> {
    ...
    securer_admin_client: SecurerAdminClient<'a>,
    // sac_address: Address,
    securer_address: Address,
...
fn set_up<'a>() -> Environment<'a>{
    ...
    let securer_address = env.register_contract(None, SecurerContract);
    let securer_admin_client = SecurerAdminClient::new(&env, &securer_address);
...
fn test_init_recovery() {
    ...
    let securer_admin_client = set_up.securer_admin_client;
    let securer_address = set_up.securer_address;

    let SimpleEd25519 {keypair: admin_ed25519_keypair, bytes: admin_ed25519_bytes, .. } = get_simple_ed25519(&env);
    let SimpleEd25519 { bytes: securer_ed25519_bytes, ..} = get_simple_ed25519(&env);
    let SimpleEd25519 { keypair: sw_owner_ed25519_keypair , bytes: sw_owner_ed25519_bytes, ..} = get_simple_ed25519(&env);
    

    wallet_client.mock_all_auths().add_signer(&Signer::Ed25519(
        sw_owner_ed25519_bytes.clone(),
        None,
    ...
    let admin_multsig = SimpleMultiSig{
        signers: vec![&env,SignerPubKey::Ed25519(admin_ed25519_bytes.clone())],
        threshold: BytesN::<1>::from_array(&env, &[1]),
    };

    let securer_multisig = SimpleMultiSig{
        signers: vec![&env,SignerPubKey::Ed25519(securer_ed25519_bytes)],
        threshold: BytesN::<1>::from_array(&env, &[1]),
    };

    let add_securer_invocation = SorobanAuthorizedInvocation {
        function: SorobanAuthorizedFunction::ContractFn(InvokeContractArgs {
            contract_address: securer_address.clone().try_into().unwrap(),
            function_name: "init_securer".try_into().unwrap(),
            args: std::vec![
                admin_multsig.clone().try_into().unwrap(),
                securer_multisig.clone().try_into().unwrap(),
            ]
            .try_into()
            .unwrap(),
        }),
    ...
    let nonce = 5;
    let admin_ed25519_signature = sign_payload(&env,nonce, &admin_ed25519_keypair, signature_expiration_ledger, &add_securer_invocation);
    ...
    let init_recovery_auth = SorobanAuthorizationEntry {
        credentials: SorobanCredentials::Address(SorobanAddressCredentials {
            ...
            signature: Signatures(map![
                &env,
                (
                    SignerPubKey::Ed25519(admin_ed25519_bytes.clone()),
                    Some(admin_ed25519_signature)
                ),
            ])
            .try_into()
            .unwrap(),
        }),
        root_invocation: add_securer_invocation.clone(),
    };

    securer_admin_client.set_auths(&[init_recovery_auth]).init_securer(
        &admin_multsig,
        &securer_multisig,
    );
